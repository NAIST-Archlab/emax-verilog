...
// assign axi_s_wready  = axiif_sbusy && axiif_srw &&              (axiif_aligned_addr == `REG_BASE_PHYS ? !axiif_bufq : (axiif_blen == axiif_ilen && (c2c_drive_m_wr?axiif_mlen==axiif_ilen:1'b1)));
   reg 			       sr_wr_cyc;
   assign axi_s_wready  = axiif_sbusy && axiif_srw && sr_wr_cyc && (axiif_aligned_addr == `REG_BASE_PHYS ? !axiif_bufq : (axiif_blen == axiif_ilen && (c2c_drive_m_wr?axiif_mlen==axiif_ilen:1'b1)));
   always @(posedge ACLK or negedge RSTN) begin
     if (~RSTN) begin
       sr_wr_cyc	<=	1'b0;
     end else if( axi_s_wready == 1'b1 && axi_s_wvalid == 1'b1 && axi_s_wlast == 1'b1 && sr_wr_cyc == 1'b1 )begin
       sr_wr_cyc	<=	1'b0;
     end else if( axi_s_awvalid == 1'b1 && axi_s_awready == 1'b1 && sr_wr_cyc == 1'b0 )begin
       sr_wr_cyc	<=	1'b1;
     end
   end
//// FT 2023/12/09 END
...
//// FT 2020/09/28 END
   ...
   assign axring_top_addr = (axring_top_ty == 3'd4) ? vadr[31:0]:axiif_aligned_addr[31:0];
